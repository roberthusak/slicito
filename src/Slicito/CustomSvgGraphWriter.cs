...
namespace Slicito
{
    ///<summary>
    ///</summary>
    internal class CustomSvgGraphWriter
    {
        ...
        void Open()
        {
            WriteComment("SvgWriter version " + typeof(CustomSvgGraphWriter).Assembly.GetName().Version);
        ...
        public static void WriteAllExceptEdges(Graph graph, string outputFile) {
            using (var stream = File.Create(outputFile)) {
                var writer = new CustomSvgGraphWriter(stream, graph) {
                    Precision = 4,
                    IgnoreEdges = true
                };
                writer.Write();
